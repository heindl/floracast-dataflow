...
def transformed_input_fn(transformed_location, mode, batch_size=50, epochs=1, raw_data_file_pattern=None):
    ...
    fn = input_fn_maker.build_transforming_training_input_fn(
        ...
        transformed_label_keys=([constants.KEY_OCCURRENCE_ID] if mode == estimator.ModeKeys.PREDICT else [constants.KEY_TAXON]),
        transformed_feature_keys=all_feature_keys(),
    ...
    for label in list_feature_keys():
    ...
    def _get_model_fn(estimator):
        ...
        def _model_fn(features, labels, mode, config):
            if mode == tf.estimator.ModeKeys.PREDICT:
                key = features.pop(constants.KEY_OCCURRENCE_ID)
            ...
            if mode == tf.estimator.ModeKeys.PREDICT:
                model_fn_ops.predictions[constants.KEY_OCCURRENCE_ID] = key
